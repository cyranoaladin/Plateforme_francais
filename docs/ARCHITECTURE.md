...
Serveur MCP autonome dans `packages/mcp-server/`. Expose 20 outils pour les agents IA (consultation de ressources, interrogation de la base, exécution de skills LLM). Communique via HTTP (`MCP_SERVER_URL`), authentifié par `MCP_API_KEY`.
