...
describe('Quotas Single Source of Truth', () => {
  ...
  it('MAX legacy alias should resolve to PRO quotas', () => {
    const maxAlias = getPlanConfig('MAX');

    expect(maxAlias.id).toBe('PRO');
    expect(maxAlias.quotas.ORAL_SESSIONS).toEqual({ limit: 'unlimited', period: 'week' });
    expect(maxAlias.quotas.WRITTEN_CORRECTIONS).toEqual({ limit: 'unlimited', period: 'month' });
    expect(maxAlias.quotas.TUTOR_QUESTIONS).toEqual({ limit: 'unlimited', period: 'day' });
    expect(maxAlias.quotas.RAG_SEARCH).toEqual({ limit: 'unlimited', period: 'day' });
    expect(maxAlias.quotas.QUIZ_PER_DAY).toEqual({ limit: 'unlimited', period: 'day' });
    expect(maxAlias.quotas.OCR_COPIES).toEqual({ limit: 50, period: 'month' });
    expect(maxAlias.quotas.LLM_TOKENS).toEqual({ limit: 200_000, period: 'day' });
  });
  ...
  it('should have correct pricing', () => {
    ...
    expect(getPlanConfig('MAX').priceTnd).toBe(129);
  });
