...
curl -sS -D "$TMP_HEADERS" -o /dev/null \
  ...
  --data "{\"email\":\"${EMAIL}\",\"password\":\"${PASSWORD}\",\"displayName\":\"Contract User\",\"acceptedCgu\":true,\"cguVersion\":\"2026-03\"}"
...
ROLE_COOKIE="$(grep -i '^set-cookie: eaf_role=' "$TMP_HEADERS" | head -n1 | sed -E 's/^set-cookie: eaf_role=([^;]+).*/\1/i')"

if [ -z "${SESSION_COOKIE}" ] || [ -z "${CSRF_COOKIE}" ] || [ -z "${ROLE_COOKIE}" ]; then
...
docker run --rm \
  ...
  --phases fuzzing \
  --header "Cookie: eaf_session=${SESSION_COOKIE}; eaf_csrf=${CSRF_COOKIE}; eaf_role=${ROLE_COOKIE}" \
