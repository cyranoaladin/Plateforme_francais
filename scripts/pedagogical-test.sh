...
if [ -d "$APP_DIR/.next/server" ]; then
  if grep -r "Introduction.*pts\|Conclusion.*pts" "$APP_DIR/.next/server/" 2>/dev/null | grep -iv "history\|introd.*text\|node_modules" | head -3 | grep -q .; then
  ...
  warn "Répertoire .next/server absent — skip check build"
fi
...
if [ -d "$APP_DIR/.next/server" ]; then
  if grep -r "Interpréter.*effet\|interprétation.*gramm\|gramm.*interprét" "$APP_DIR/.next/server/" 2>/dev/null | grep -iv "node_modules\|ne pas\|pas de\|n'a pas\|jamais" | head -3 | grep -q .; then
...
if [ -d "$APP_DIR/.next/server" ]; then
  if grep -r "2019-042" "$APP_DIR/.next/server/" 2>/dev/null | grep -iv "node_modules" | head -1 | grep -q .; then
    green "Référence Note de Service 2019-042 présente dans le build"
  else
    red "Note de Service 2019-042" "absente du build"
  fi
else
  warn "Répertoire .next/server absent — skip"
fi
...
echo "── Anti-triche ─────────────────────────────────────────"
if [ -d "$APP_DIR/.next/server" ]; then
  if grep -ri "ne jamais fournir" "$APP_DIR/.next/server/" 2>/dev/null | grep -iv "node_modules" | head -1 | grep -q .; then
