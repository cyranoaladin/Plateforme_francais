...
export function CountdownBadge({ className }: CountdownBadgeProps) {
  ...
  if (days === null) {
    return (
      <div
        className={`inline-flex items-center gap-2 rounded-full border border-gold-500 bg-gold-50 px-4 py-1.5 text-sm font-medium text-gold-700 animate-pulse ${className ?? ''}`}
        aria-hidden="true"
      >
        <span className="inline-block h-4 w-32 rounded bg-gold-100" />
      </div>
    );
  }
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 rounded-full border border-gold-500 bg-gold-50 px-4 py-1.5 text-sm font-medium text-gold-700 ${className ?? ''}`}
      aria-live="polite"
    >
